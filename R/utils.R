...
##
## - `plot_heatmap_correlation_deconvolution_vs_true_Edec()`: Plots a heatmap of
##    the correlation between the estimated methylation profiles and the
...
plot_deconvolution_performance <- function(running_info, title_addition = ''){
  ...
  iteration <- divergence_value <- delta_divergence_value <- NULL

  # Iterations vs divergence value
  ggplot2::ggplot(running_info.df,
                  ggplot2::aes(iteration, log(divergence_value))) +
    ggplot2::geom_line() + ggplot2::theme_bw() +
    ggplot2::ggtitle("Iterations vs log(divergence)")

  # Iterations vs delta
  ggplot2::ggplot(running_info.df,
                  ggplot2::aes(iteration, log(delta_divergence_value))) +
    ggplot2::geom_line() + ggplot2::theme_bw() +
  ...
  ggplot2::ggplot(running_info.df, ggplot2::aes(x=iteration)) +
    ggplot2::geom_line(ggplot2::aes(y = log(divergence_value),
                  color = 'divergence_value')) +
    ggplot2::geom_line(ggplot2::aes(y = log(delta_divergence_value),
...
#' @param verbose Visualize additional information of the models added. Default
#'  value FALSE.
#'
#' @details
...
plot_heatmap_alpha_beta <- function(
  ...
  color_combination_gradient <- grDevices::colorRampPalette(RColorBrewer::brewer.pal(10, "RdYlBu"))(256)
...
plot_deconvolution_performance_x_y_z_total <- function(
  ...
  # Solution for the error 'no visible binding for global variable' posted in:
  # https://www.r-bloggers.com/2019/08/no-visible-binding-for-global-variable/
  iteration <- divergence_value_x <- divergence_value_y <- alpha_scale <-
    divergence_value_z <- beta_scale <- divergence_value <-
    delta_divergence_value <- colors_by_lines <- NULL

  # Plot with both metric included
  ggplot2::ggplot(running_info.df, ggplot2::aes(x=iteration)) +
    ggplot2::geom_line(ggplot2::aes(y = log(divergence_value_x),
                  color = 'divergence_value_x'),
              linetype = 'solid', size = 1) +
    ggplot2::geom_line(ggplot2::aes(y = log(divergence_value_y * alpha_scale),
                  color = 'divergence_value_y'),
              linetype = 'solid', size = 1) +
    ggplot2::geom_line(ggplot2::aes(y = log(divergence_value_z * beta_scale),
    ...
    ggplot2::ylab('log(metric)') +
    ggplot2::theme_bw() +
    ggplot2::ggtitle(paste0("Iterations vs log(divergence) and log(delta divergence)",
                   ' - ', title_addition)) +
    ggplot2::theme(plot.title = ggplot2::element_text(size=10))
}
