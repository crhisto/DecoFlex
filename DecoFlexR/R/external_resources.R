...
#'
#' @export
create_basis_multiple <- function(x, ct.sub = NULL, ct.varname, sample,
...
#'
#' @export
create_basis_one <- function(x , ct.sub = NULL, ct.varname, sample,
                                 ct.cell.size = NULL, verbose = FALSE){
  # select only the subset of cell types of interest
  if (is.null(ct.sub)){
...
#'   - detailed: The detailed output from create_basis_one or
...
decoflex_build_cell_reference <- function(x, ct.sub = NULL, ct.varname,
  ...
  if(use_single_sampling_reference){
    ...
    result_reference <- create_basis_one(x = x, ct.sub = ct.sub,
    ...
    result_reference <- create_basis_multiple(x = x, ct.sub = ct.sub,
                                         ct.varname = ct.varname,
                                         sample = sample,
                                         ct.cell.size = ct.cell.size,
                                         verbose = verbose)
