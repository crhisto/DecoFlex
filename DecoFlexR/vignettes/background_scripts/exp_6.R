# Deconvolution using different references to deconvolute: CA
#parameteres:
root.dir <- "/mnt_volumen/GIT_REPOSITORIES/DecoFlex/DecoFlexR/vignettes/results/exp_6/"
...
drop.exp6.complete <- drop.sub.exp6.complete <- list()

#Order based on celltype size
celltypes.exp6.merge1 <- c("Neurons", "Oligodendrocytes", "OPCs", "Astrocytes")
...
drop.exp6.complete <- foreach(j = 1:length(sc.ref.object),
                   .combine = c)  %dopar%
  {
    ...
    drop.exp6.complete.temp <- run.decoflex(single_cell_data_exp = eset.sc.sparse_dataset,
    ...
    save(drop.exp6.complete.temp, file = paste0(root.dir, dataset_name, "_reference_CA_input.exp6_Estimates.rda"))
    
    list_values_model <- list()
    list_values_model[[dataset_name]] <- drop.exp6.complete.temp
...
#Saving everything
save(drop.exp6.complete, file = paste0(root.dir, "all_reference_CA_input.exp6_Estimates.rda"))
