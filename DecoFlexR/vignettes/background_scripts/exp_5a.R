# Deconvolution using different bulk references (F5, MM, IP) to deconvolute: VL

#parameteres:
root.dir <- "/mnt_volumen/GIT_REPOSITORIES/DecoFlex/DecoFlexR/vignettes/results/exp_5a/"
...
load(paste0(root.dir, "resources_exp5_extra.rData"))
...
drop.exp5a.complete <- drop.sub.exp5a.complete <- list()

#Order based on celltype size
celltypes.exp5a.merge1 <- c("OPCs", "Microglia", "Endothelial", "Astrocytes", "Oligodendrocytes", "Neurons")
...
drop.exp5a.complete <- foreach(j = 1:length(list_bulk_signatures),
                   .combine = c)  %dopar%
  {
    ...
    gene_intersection <- intersect(rownames(eset.sparse_pseudobulk_VL@assayData$exprs), rownames(dataset_value))

    # 4. Deconvolution with the partially fixed h and w
    drop.exp5a.complete.temp <- run_deconvolution_decoflex_semi_reference(
      bulk.data_mixtures.brain = data.frame(as.matrix(eset.sparse_pseudobulk_VL@assayData$exprs[gene_intersection,])),
    ...
    save(drop.exp5a.complete.temp, file = paste0(root.dir, dataset_name, "_reference_VL_input.exp5a_Estimates.rda"))
    
    list_values_model <- list()
    list_values_model[[dataset_name]] <- drop.exp5a.complete.temp
...
#Saving everything
save(drop.exp5a.complete, file = paste0(root.dir, "all_reference_VL_input.exp5a_Estimates.rda"))
