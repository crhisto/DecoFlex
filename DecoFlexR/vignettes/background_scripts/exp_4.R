...
celltypes.exp2.merge1 <- c("OPCs", "Microglia", "Endothelial", "Astrocytes", "Oligodendrocytes", "Neurons")

celltypes.exp2.merge1.obj <- list()
celltypes.exp2.merge1.obj$one <- c("OPCs")
celltypes.exp2.merge1.obj$two <- c("OPCs", "Microglia")
celltypes.exp2.merge1.obj$three <- c("OPCs", "Microglia", "Endothelial")
celltypes.exp2.merge1.obj$four <- c("OPCs", "Microglia", "Endothelial", "Astrocytes")
celltypes.exp2.merge1.obj$five <- c("OPCs", "Microglia", "Endothelial", "Astrocytes", "Oligodendrocytes")
celltypes.exp2.merge1.obj$six <- c("OPCs", "Microglia", "Endothelial", "Astrocytes", "Oligodendrocytes", "Neurons")
...
drop.exp2.complete <- foreach(j = 1:length(celltypes.exp2.merge1.obj),
                   .combine = c)  %dopar%
  {
    
    celltype_name <- names(celltypes.exp2.merge1.obj)[j]
    celltype_list_missing <- celltypes.exp2.merge1.obj[[celltype_name]]
    ...
    rest_celltypes <- celltypes.exp2.merge1[!(celltypes.exp2.merge1 %in% celltype_list_missing)]
    ...
    save(drop.exp2.complete.temp, file = paste0(root.dir, celltype_name, "_Dropped_sub.exp4_Estimates.rda"))
...
#Saving everything
save(drop.exp2.complete, file = paste0(root.dir, "all_Dropped_sub.exp4_Estimates.rda"))
