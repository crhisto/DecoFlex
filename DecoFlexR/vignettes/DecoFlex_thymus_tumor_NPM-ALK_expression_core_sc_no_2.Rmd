...
```{r}
#save(list = ls(all.names = TRUE, name = environment()), file = "/Users/crhisto/Documents/GitHub/DecoFlex/DecoFlexR/vignettes/figures_manuscript/backup/environment_13_10_2023.RData")

#load("/Users/crhisto/Documents/GitHub/DecoFlex/DecoFlexR/vignettes/figures_manuscript/backup/environment_27_09_2023.RData")
```
...
hierarchical_clustering_thymus.orig.celltype.v3 <- list(
  '1'=list(tree_level=0, leaf_number=1, parameters=list(param.logfc.threshold = 1.0,
                                                        param.p_val_adj = 0.05,
                                                        minimum_markers = 4,
                                                        min_delta_cor_threshold = 0.3), 
...
Plot of correlation vs marker gene selected in the same axes X (y=correlation, x = count)
```{r function.plot_correlation_iterations}
plot_correlation_iterations <- function(data.correlation.all, 
  ...
  if(!is.null(red_line)){
    correlation.vs.markers.selected.plot <- correlation.vs.markers.selected.plot + geom_vline(xintercept=red_line,linetype = "dashed", colour = "red")
  }
  
  if(!is.null(black_line)){
    correlation.vs.markers.selected.plot <- correlation.vs.markers.selected.plot + geom_vline(xintercept=black_line,linetype = "dashed", colour = "black")
  }
...
Initially I tried the default values param.logfc.threshold = 1.0, however the minimum correlation was too high .66, maybe due to the sc experiment (depth, number of nuclei, quality of experiment, etc), so I have to tried lower values in order to get more representative markers, even trying lower p-adj value. The best cases was 60 markers(52) and 0.05. However, the minimum seems not to be the best results. 13-88
```{r marker.analysis.thymus.merge1}

#Parameters for the simulation
single_cell_data_exp <- eset.sc.sparse_thymus
hierarchy = hierarchical_clustering_thymus.orig.celltype.v0$`1`
...
marker.genes.OMiC.thymus.merge1 <- 
...
```{r correlation.vs.markers.selected.plot.thymus.merge1, fig.width=7, fig.height=5}
library(ggplot2)
data.correlation.all.thymus.merge1 <- marker.genes.OMiC.thymus.merge1$accumulative_corr_markers
#Ordering celltypes
order_celltypes.thymus.merge1 <- c("1DN", "2preDP", "3DPbla", "4DPre", "5DPsel", "6CD4sp", "7CD8sp")
data.correlation.all.thymus.merge1$celltype <- factor(data.correlation.all.thymus.merge1$celltype, levels = order_celltypes.thymus.merge1)

#Creation of the plot
correlation.vs.markers.selected.plot.thymus.merge1 <- plot_correlation_iterations(data.correlation.all.thymus.merge1, zoom.xlim = c(0, 20), red_line = 9, black_line = 13)
plot(correlation.vs.markers.selected.plot.thymus.merge1)

#save_plot_as_pdf(correlation.vs.markers.selected.plot.thumus.merge1, 'thymus/correlation.vs.markers.selected.plot.thymus.merge1.pdf', 7, 5)
```


```{r plots.hierarchy.function}

plot_correlation_iterations.hierarchy <- function(
  ...
  message(paste0('plot_correlation_iterations.hierarchy - Verbose: ',
                 verbose))
  ...
  #2. Checking markers
  marker.genes.OMiC.result <- 
  run_marker_selection_OMiC(single_cell_data_exp = single_cell_data_exp, 
                            hierarchy = hierarchy,
                            sub_clusters_var = sub_clusters_var,
                            sample = sample, 
                            use_min_cor_strategy = use_min_cor_strategy, 
  ...
  # 3.1. Calculation of minimum correlation marker
  minimum.markers <- length(marker.genes.OMiC.result$total_markers)
  marker_filtered <-  marker.genes.OMiC.result$accumulative_corr_markers[ marker.genes.OMiC.result$accumulative_corr_markers$current_marker == minimum.markers,]
  id_selected <- rownames(marker_filtered)[nrow(marker_filtered)]
  batch <- max(marker.genes.OMiC.result$accumulative_corr_markers$marker_count[1:id_selected])
  
  print(paste0('Batch: ', batch, ', Minimum, markers: ', minimum.markers))
  
  # 3. I will plot the corresponding marker behavior
  correlation.vs.markers.selected.plot <- plot_correlation_iterations(
    marker.genes.OMiC.result$accumulative_corr_markers,
    zoom.xlim = c(0, round(max(marker.genes.OMiC.result$accumulative_corr_markers$marker_count)*.25)),
    red_line = batch, 
    black_line = NULL)
  plot(correlation.vs.markers.selected.plot)
  

  #4. checking all the hierarchy parts
  result_markers.OMiC_subclusters = NULL
  for(current_hierarchy in hierarchy$next_level_clustering) {
    ...
    # 4.1. check the subcluster that are part of the top cluster analyzed.
    ...
    # 4.2. This is just a double check since both list must coincide.
    ...
    # 4.3. Checking for next level.
    marker.genes.OMiC.results <- NULL
    if(length(subclusters_list) > 1){
        marker.genes.OMiC.results <- 
          plot_correlation_iterations.hierarchy(
            single_cell_data_exp = single_cell_data_exp, 
    ...
    # 4.4. Add the results to the global results.
    result_markers.OMiC_subclusters[[top_cluster]] <- marker.genes.OMiC.results
    
  }
  
  return(list(
    marker.genes.OMiC.result = marker.genes.OMiC.result,
    correlation.vs.markers.selected.plot = correlation.vs.markers.selected.plot, 
    next_level = result_markers.OMiC_subclusters
  ))
}
...
```{r hierarchy.marker.analysis.thymus.merge1, fig.width=7, fig.height=5}

#Parameters for the simulation
single_cell_data_exp <- eset.sc.sparse_thymus
hierarchy = hierarchical_clustering_thymus.orig.celltype.optimized$`1`
bulk_data = data.frame(as.matrix(eset.sparse_pseudobulk_thymus@assayData$exprs))
sub_clusters_var <- 'orig.celltype'
sample <- 'Disorder' #just one value: Control
use_min_cor_strategy = TRUE
...
#let's run de simulation
hierarchy.marker.genes.OMiC.thymus.merge1 <- 
  plot_correlation_iterations.hierarchy(
                            single_cell_data_exp = single_cell_data_exp, 
                            hierarchy = hierarchy,
                            sub_clusters_var = sub_clusters_var,
...
```{r hierarchy.optimized}

hierarchical_clustering_thymus.orig.celltype.optimized <- list(
  '1'=list(tree_level=0, leaf_number=1, parameters=list(param.logfc.threshold = 0.1,
                                                        param.p_val_adj = 0.05,
                                                        minimum_markers = 4,
                                                        min_delta_cor_threshold = 0.0), 
           celltype_list=list_cell_types_thymus.orig.celltype, 
           next_level_clustering=list(
             '1'=list(tree_level=1, leaf_number=1, name = 'DN', celltype_list=c("1DN"), next_level_clustering=NULL), 
             '2'=list(tree_level=1, leaf_number=2, name = 'DP', parameters=list(param.logfc.threshold = 0.1,
                                                                   param.p_val_adj = 0.05,
                                                                   minimum_markers = 4,
                                                                   min_delta_cor_threshold = 0.0),  
                      celltype_list=c("2preDP", "3DPbla", "4DPre", "5DPsel"), next_level_clustering=NULL),
             '3'=list(tree_level=1, leaf_number=3, name = 'CDs',parameters=list(param.logfc.threshold = 0.1,
                                                                   param.p_val_adj = 0.05,
                                                                   minimum_markers = 4,
                                                                   min_delta_cor_threshold = 0.0),  
                      celltype_list=c("6CD4sp", "7CD8sp"), next_level_clustering=NULL)
...
select_markers <- function(marker.genes.OMiC_value, marker_number){
  marker.genes.OMiC <- list()
  marker.genes.OMiC$accumulative_corr_markers <- marker.genes.OMiC_value$accumulative_corr_markers
  marker.genes.OMiC$list_markers <- NULL
  marker.genes.OMiC$list_markers.percentile <- NULL
  
  
    #Adding just the markers that I'm interesting on.
    marker_filtered <-  marker.genes.OMiC_value$accumulative_corr_markers[ marker.genes.OMiC_value$accumulative_corr_markers$current_marker == marker_number,]
    id_selected <- rownames(marker_filtered)[nrow(marker_filtered)]
    
    marker.genes.OMiC$total_markers <- unique(marker.genes.OMiC_value$accumulative_corr_markers$gene[1:id_selected])
    
    return(marker.genes.OMiC)
}

#Let's get the objects for the new limits
marker.genes.thymus.merge1.top <- select_markers(
  marker.genes.OMiC_value=hierarchy.marker.genes.OMiC.thymus.merge1$marker.genes.OMiC.result,
  marker_number=37)
marker.genes.thymus.merge1.dp <- select_markers(
  marker.genes.OMiC_value=hierarchy.marker.genes.OMiC.thymus.merge1$next_level$subcluster_level_1_leaf_2$marker.genes.OMiC.result, 
  marker_number=43)
marker.genes.thymus.merge1.cds <- select_markers(
  marker.genes.OMiC_value=hierarchy.marker.genes.OMiC.thymus.merge1$next_level$subcluster_level_1_leaf_3$marker.genes.OMiC.result, 
  marker_number=8)

hierarchical_clustering_thymus.orig.celltype.optimized.v1 <- list(
  '1'=list(tree_level=0, leaf_number=1, parameters=list(param.logfc.threshold = 0.1,
                                                        param.p_val_adj = 0.05,
                                                        minimum_markers = 4,
                                                        min_delta_cor_threshold = 0.0, 
                                                        markers.clusters.parameter = marker.genes.thymus.merge1.top), 
           celltype_list=list_cell_types_thymus.orig.celltype, 
           next_level_clustering=list(
             '1'=list(tree_level=1, leaf_number=1, name = 'DN', celltype_list=c("1DN"), next_level_clustering=NULL), 
             '2'=list(tree_level=1, leaf_number=2, name = 'DP', parameters=list(param.logfc.threshold = 0.1,
                                                                                ...
                                                                                minimum_markers = 4,
                                                                                min_delta_cor_threshold = 0.0, 
                                                                                markers.clusters.parameter = marker.genes.thymus.merge1.dp),  
                      celltype_list=c("2preDP", "3DPbla", "4DPre", "5DPsel"), next_level_clustering=NULL),
             '3'=list(tree_level=1, leaf_number=3, name = 'CDs',parameters=list(param.logfc.threshold = 0.1,
                                                                                param.p_val_adj = 0.05,
                                                                                minimum_markers = 4,
                                                                                min_delta_cor_threshold = 0.0, 
                                                                                markers.clusters.parameter = marker.genes.thymus.merge1.cds),  
                      celltype_list=c("6CD4sp", "7CD8sp"), next_level_clustering=NULL)
           )
  )
)
...
The better value after the minimum is: 88 (76 unique markers) with a correlation value of: 0.49398321
```{r better_value.deco}

  markers.clusters.object.thymus.merge1 <- list()
  markers.clusters.object.thymus.merge1$accumulative_corr_markers <- marker.genes.OMiC.thymus.merge1$accumulative_corr_markers
  markers.clusters.object.thymus.merge1$list_markers <- NULL
  markers.clusters.object.thymus.merge1$list_markers.percentile <- NULL
  
  #Adding just the markers that I'm interesting on.
  marker_filtered <-  marker.genes.OMiC.thymus.merge1$accumulative_corr_markers[ marker.genes.OMiC.thymus.merge1$accumulative_corr_markers$current_marker == 88,]
  id_selected <- rownames(marker_filtered)[nrow(marker_filtered)]
  markers.clusters.object.thymus.merge1$total_markers <- unique(marker.genes.OMiC.thymus.merge1$accumulative_corr_markers$gene[1:id_selected])
...
hierarchy = hierarchical_clustering_thymus.orig.celltype.v3$`1`
bulk_data = data.frame(as.matrix(eset.sparse_pseudobulk_thymus@assayData$exprs))
sub_clusters_var <- 'orig.celltype'
...
delete_shared_internal_markers = FALSE
ordering_strategy = 'foldchange_pvalue'
#markers.clusters.parameter = markers.clusters.object.thymus.merge1
...
#save(deco.actual.data.thymus.orig.celltype, file = paste0(root.dir, "deco.actual.data.thymus.orig.celltype_flat.hierarchy.132.24.8_66.rDAta"))

```

```{r performance.plot.thymus.mixture, fig.width=4, fig.height=4}
#Checking just for divergences
thymus.plot_performance.thymus.mixture <- plot_deconvolution_performance_x_y_z_total(
  deco.actual.data.thymus.orig.celltype$result_deco_top_cluster$running_info,
  alpha=0,
  beta = 0,
  ...
  include_zoom = FALSE,
  zoom_xlim_limits=c(400, 475), 
  include_title = FALSE) + theme(legend.position = "top")

plot(thymus.plot_performance.thymus.mixture)

#save_plot_as_pdf(thymus.plot_performance.thymus.mixture, 'performance_figures/thymus.plot_performance.thymus.mixture.pdf', 4, 4)
...
#new approach: minimum optimum: 0.716 default
#marker gene: 88 72.4
# 0.653 with 21, 18, 6
# 0.657 with 37, 24, 8
# 0.66 with 132, 24, 8
...
plot.formal.nmae <- function(merge, errors, dataset_name, fills.colours, celltype_order=NULL, legend.position = 'none') {
  ...
  ggplot(plot.data, aes(x = Celltype, y = Algorithm, fill = nmae, label = Value)) +
    ...
    theme_bw() +
    theme(legend.position = legend.position, panel.border = element_blank(), axis.text.x = element_text(angle = 30, hjust = 1),
...
```{r nmae.thymus.orig.celltype, fig.width=8, fig.height=2}
m2.order <- list_cell_types_thymus.orig.celltype.order

path.figures <- '/Users/crhisto/Documents/GitHub/DecoFlex/DecoFlexR/vignettes/figures_manuscript/mixture_thymus/'

#pdf(file = paste0(path.figures, "results_merge1_thymus_B.pdf"), height = 2.0, width = 8)
plot.formal.nmae(merge='orig.celltype', errors = errors.thymus.orig.celltype, dataset_name = "thymus",
                 ...
                 celltype_order = list_cell_types_thymus.orig.celltype.order) + theme(legend.position = "bottom")
#dev.off()
```
...
y <- -0.05
#pdf(file = paste0(path.figures, "results_merge1_thymus_A.pdf"), height = 3, width = 8)
...
#pdf(file = paste0(path.figures, "results_merge1_thymus_C.pdf"), height = 3, width = 5)
...
#pdf(file = paste0(path.figures, "results_merge1_thymus_D.pdf"), height = 7, width = 4)
...
reference_w.9.celltypes.thymus.with.names <- reference_w.9.celltypes.thymus$basis[, paste0('cluster_', c(1:4))]
colnames(reference_w.9.celltypes.thymus.with.names) <- cluster_list.thymus.bigstage$bigstage[c(1:6)]
...
library(DecoFlex)
#readRenviron('/Users/crhisto/.Renviron')
...
```{r performance.plot.TOP, fig.width=4, fig.height=5}
#Checking just for divergences
thymus.plot_performance.top <- plot_deconvolution_performance_x_y_z_total(
  deco.actual.data.thymus.reduced.results.top$result_deco_top_cluster$running_info,
  alpha=0,
  beta = 0,
  scale_parameters = TRUE,
  include_divergences_x_y_z = FALSE,
...
plot(thymus.plot_performance.top)

#save_plot_as_pdf(thymus.plot_performance.top, 'performance_figures/thymus.plot_performance.top.pdf', 4, 5)
...
```{r performance.plot.DP, fig.width=4, fig.height=5}
#Checking just for divergences
thymus.plot_performance.dp <- plot_deconvolution_performance_x_y_z_total(
  deco.actual.data.thymus.reduced.results.dp$result_deco_top_cluster$running_info,
  alpha=0,
  beta = 0,
  scale_parameters = TRUE,
  include_divergences_x_y_z = FALSE,
  include_divergences = FALSE,
  include_zoom = TRUE,
  zoom_xlim_limits=c(550, 680), 
  include_title = FALSE) + theme(legend.position = "top")

plot(thymus.plot_performance.dp)

#save_plot_as_pdf(thymus.plot_performance.dp, 'performance_figures/thymus.plot_performance.dp.pdf', 4, 5)
...
```{r performance.plot.CDs, fig.width=4, fig.height=5}
#Checking just for divergences
thymus.plot_performance.cds <- plot_deconvolution_performance_x_y_z_total(
  deco.actual.data.thymus.reduced.results.cds$result_deco_top_cluster$running_info,
  alpha=0,
  beta = 0,
  scale_parameters = TRUE,
  include_divergences_x_y_z = FALSE,
  include_divergences = FALSE,
  include_zoom = TRUE,
  zoom_xlim_limits=c(150, 185), 
  include_title = FALSE) + theme(legend.position = "top")

plot(thymus.plot_performance.cds)

#save_plot_as_pdf(thymus.plot_performance.cds, 'performance_figures/thymus.plot_performance.cds.pdf', 4, 5)
...
#Parameters for the simulation
single_cell_data_exp <- eset.sc.thymus.6.8.w.sparse.metacluster
hierarchy = hierarchical_clustering_sc_thymus.reduced.flat.complete
sub_clusters_var <- 'cluster_normalized_bigstage'
sample <- 'age'
use_min_cor_strategy = TRUE
delete_shared_level_markers = FALSE
...
```{r function.plot.multiple.deconvolutions}
plot_deconvolution_performance_multiple_deconvolutions <- function(
    running_info,
    title_addition = '',
    include_zoom = FALSE,
  ...
  plot <- ggplot2::ggplot(running_info.df, 
                          ggplot2::aes(x=.data$iteration,
                                       y = log(.data$delta_divergence_value),
                                       color = .data$model)) +
    ggplot2::geom_line() +
    ggplot2::ylab('log(delta_divergence_value)') +
    ggplot2::theme_bw() +
...
```{r}
#let's join different plots

running_info_top <- data.frame(deco.actual.data.thymus.reduced.results.top$result_deco_top_cluster$running_info)
running_info_top$model <- 'Top level'

running_info_dp <- data.frame(deco.actual.data.thymus.reduced.results.dp$result_deco_top_cluster$running_info)
running_info_dp$model <- 'DP level'

running_info_cds <- data.frame(deco.actual.data.thymus.reduced.results.cds$result_deco_top_cluster$running_info)
running_info_cds$model <- 'CDs level'

#Join data
running_info_joined <- rbind(running_info_top, running_info_dp)
running_info_joined <- rbind(running_info_joined,running_info_cds)
running_info_joined$model <- factor(running_info_joined$model, levels = c("Top level", "DP level", "CDs level"))
...
```{r performance.plot.complete, fig.width=4, fig.height=3}
#Checking just for divergences
thymus.plot_performance.complete <- plot_deconvolution_performance_multiple_deconvolutions(
  running_info_joined,
  include_zoom = FALSE,
  zoom_xlim_limits=c(150, 185), 
  include_title = FALSE) + theme(legend.position = "right")

plot(thymus.plot_performance.complete)

#save_plot_as_pdf(thymus.plot_performance.complete, 'performance_figures/thymus.plot_performance.complete.pdf', 4, 3)

```

#### 3.4.2.1.1 Plot of marker genes.

```{r}

ggplot(deco.actual.data.thymus.reduced.results.top$markers.top.clusters.object$list_markers, aes(x=year, y=n, group=name, color=name)) +
    geom_line()                                                                                                           
...
plot_heatmap_markers_multiple_level <- function(
  ...
  show_legends = TRUE, 
  show_row_names.param = TRUE, 
  col_function_main_values = NULL){
  ...
  heatmap.two.groups  <- Heatmap(data, name = "Markers",
                                 ...
                                 show_row_names = show_row_names.param,
...
length(unique(all_markers.thymus))
...
length(unique(all_markers.thymus.top))
...
length(unique(all_markers.thymus.dp))
...
length(unique(all_markers.thymus.cds))
```
...
limits.99.95 <- c(7.6473552, 8.854265150, 7.6399175) #35
limits.99.9 <- c(7.1600693, 8.016101030, 6.8832673) #75
limits.99.8 <- c(6.5093589, 6.955386661, 6.0556698) # 192
limits.99.8_filtered <- c(6.5093589, 8.016101030, 6.0556698) # 192
limits.99.7 <- c(5.9384472, 6.275647237, 5.3278772) # 313
limits.99.5 <- c(5.1226535, 5.506788817, 4.5012223) #524 genes
limits.99.5_filtered <- c(5.1226535, 8.016101030, 4.5012223) #with middle as 99.8
limits.99 <- c(4.1945914, 4.545413862, 3.6603378)
limits.99.filtered <- c(4.1945914, 6.955386661, 3.6603378)

limits <- limits.99.5

#61 all de genes.
de_genes.thymus <- c(select_markers_de(RES.thymus.lfc$treatment_Thy_KO_vs_Thy_WT, limits[1]),
                     select_markers_de(RES.thymus.lfc$treatment_Thy_ALKpos_WT_vs_Thy_WT, limits[2]),
                     select_markers_de(RES.thymus.lfc$treatment_Thy_ALKpos_KO_vs_Thy_WT, limits[3]))
length(unique(de_genes.thymus))


# just ko.
#de_genes.thymus <- c(select_markers_de(RES.thymus.lfc$treatment_Thy_KO_vs_Thy_WT, 4.1945914),
#                     select_markers_de(RES.thymus.lfc$treatment_Thy_ALKpos_KO_vs_Thy_WT, 3.6603378))
#length(de_genes.thymus)

#All genes
de_genes.thymus.treatment_Thy_KO_vs_Thy_WT <- c(select_markers_de(RES.thymus.lfc$treatment_Thy_KO_vs_Thy_WT, limits[1])) #6
paste(shQuote(de_genes.thymus.treatment_Thy_KO_vs_Thy_WT, type="sh"), collapse=", ")
length(de_genes.thymus.treatment_Thy_KO_vs_Thy_WT)


#check with this values 8.016101030
de_genes.thymus.treatment_Thy_ALKpos_WT_vs_Thy_WT <- c(select_markers_de(RES.thymus.lfc$treatment_Thy_ALKpos_WT_vs_Thy_WT, limits[2]))#8
...
#old value
de_genes.thymus.treatment_Thy_ALKpos_KO_vs_Thy_WT <- c(select_markers_de(RES.thymus.lfc$treatment_Thy_ALKpos_KO_vs_Thy_WT, limits[3]))
paste(shQuote(de_genes.thymus.treatment_Thy_ALKpos_KO_vs_Thy_WT, type="sh"), collapse=", ")
length(de_genes.thymus.treatment_Thy_ALKpos_KO_vs_Thy_WT)
...
#with 99.9 = 65 (7.1600693, 8.016101030, 6.8832673) 
#with 99.5 = 529 (5.1226535, 5.506788817, 4.5012223) setting 8.016101030 = 106
#with 99 =  988 (4.1945914, 4.545413862, 3.6603378) setting 99.8 = 118
...
quantile(abs(as.data.frame(treatment_Thy_KO_vs_Thy_WT_res)$log2FoldChange), c(.25, .50, .75, .90, .95, .99, .995, .997, .998, .999, .9995), na.rm = TRUE) 
...
quantile(as.data.frame(res_treatment_Thy_ALKpos_WT_vs_Thy_WT)$log2FoldChange, c(.25, .50, .75, .90, .95, .99, .995, .997, .998, .999, .9995), na.rm = TRUE) 
...
quantile(abs(as.data.frame(treatment_Thy_ALKpos_KO_vs_Thy_WT_res)$log2FoldChange), c(.25, .50, .75, .90, .95, .99, .995, .997, .998, .999, .9995), na.rm = TRUE) 
...
#de_genes.thymus.total <- de_genes.thymus.treatment_Thy_KO_vs_Thy_WT #1
#de_genes.thymus.total <- de_genes.thymus.treatment_Thy_ALKpos_WT_vs_Thy_WT #2
#de_genes.thymus.total <- de_genes.thymus.treatment_Thy_ALKpos_KO_vs_Thy_WT #3
...
```{r creation.mask.matrices.h.w.one.celltype.v0}
...
```{r creation.mask.matrices.h.w.one.celltype.v1}
...
```{r creation.mask.matrices.h.w.one.celltype.v2}
...
```{r}
#524 genes
#deco.alpha.beta.zero.results.partial.top.extended <- deco.alpha.beta.zero.results.partial.top
```
...
#load(paste0('/Users/crhisto/Documents/GitHub/DecoFlex/DecoFlexR/vignettes/figures_manuscript/', 'deco.alpha.beta.zero.results.partial.top_Thy_KO_vs_Thy_WT.Rdata'))
```
...
plot(plot_proportions_top_semireference.2.unknown)

#save_plot_as_pdf(plot_proportions_top_semireference.2.unknown, 'plot_proportions_top_semireference.2.unknown.pdf', 8, 5)

```

Normal version
```{r heatmap.level.top.semireference, fig.height=15, fig.width=15}
...
plot.level.top.semireference <- plot_heatmap_markers_multiple_level(
                     ...
                     scale_activated = TRUE, 
                     show_legends = TRUE, 
                     show_row_names.param = FALSE
                     ) 

plot(plot.level.top.semireference$plot)

#save_plot_as_pdf(plot.level.top.semireference$plot, 'plot.level.top.semireference_de_join_extended_524_markers.pdf', 15, 40)
#save_plot_as_pdf(plot.level.top.semireference$plot, 'plot.level.top.semireference_de_join_extended_524_markers.pdf', 15, 40)
...
Improved version
Version with unknown subclusters ordered and new palette
```{r heatmap.level.top.semireference.unknown.clusters, fig.height=15, fig.width=15}

top.level.1.markers.detailed_all.unknown <- data.frame(cbind(extra_marker_genes_semireference, 'Additional Markers', TRUE))
colnames(top.level.1.markers.detailed_all.unknown) <- colnames(top.level.1.markers.detailed_all$top.markers.detailed_all)
rownames(top.level.1.markers.detailed_all.unknown) <- top.level.1.markers.detailed_all.unknown$gene_name
...
#Adding those to the new one
top.level.1.markers.detailed_all.plus.groups <- rbind(top.level.1.markers.detailed_all$top.markers.detailed_all,
                                                      top.level.1.markers.detailed_all.groups)

#Plot with heatmap level 1
library(circlize)
col_fun = colorRamp2(c(-0.6, 0, 0.6), c("blue", "white", "red"))
plot.level.top.semireference.final <- plot_heatmap_markers_multiple_level(
                     sc.eset.metacluster.all = eset.sc.thymus.6.8.w.sparse.metacluster,
                     sc.bulk.data.filtered.tpm = eset.thymus.bulk.sparse.df.filtered.tpm,
                     pseudo.bulk.df.filtered.tpm = sc.pseudo.bulk.data.top,
                     res.object.de = RES.thymus.lfc$treatment_Thy_ALKpos_KO_vs_Thy_WT, 
                     ...
                     show_legends = TRUE, 
                     show_row_names.param = FALSE, 
                     col_function_main_values = col_fun
                     ) 

save_plot_as_pdf(plot.level.top.semireference.final$plot, 'plot.level.top.semireference.final_additional_markers.pdf', 15, 15)
...
group.3 <- unlist(order_groups$'3')
group.4 <- unlist(order_groups$'4')
group.5 <- unlist(order_groups$'5')
...
group.3.names <- plot.level.top.semireference_semireference@row_names_param$labels[group.3]
paste(shQuote(group.3.names, type="sh"), collapse=", ")

group.4.names <- plot.level.top.semireference_semireference@row_names_param$labels[group.4]
paste(shQuote(group.4.names, type="sh"), collapse=", ")

group.5.names <- plot.level.top.semireference_semireference@row_names_param$labels[group.5]
paste(shQuote(group.5.names, type="sh"), collapse=", ")
...
#Creation of genes for the new unknown celltype
top.level.1.markers.detailed_all.group.1 <- data.frame(cbind(group.1.names, 'Group 1', TRUE))
colnames(top.level.1.markers.detailed_all.group.1) <- colnames(top.level.1.markers.detailed_all$top.markers.detailed_all)
top.level.1.markers.detailed_all.group.2 <- data.frame(cbind(group.2.names, 'Group 2', TRUE))
colnames(top.level.1.markers.detailed_all.group.2) <- colnames(top.level.1.markers.detailed_all$top.markers.detailed_all)

top.level.1.markers.detailed_all.group.3 <- data.frame(cbind(group.3.names, 'Group 3', TRUE))
colnames(top.level.1.markers.detailed_all.group.3) <- colnames(top.level.1.markers.detailed_all$top.markers.detailed_all)

top.level.1.markers.detailed_all.group.4 <- data.frame(cbind(group.4.names, 'Group 4', TRUE))
colnames(top.level.1.markers.detailed_all.group.4) <- colnames(top.level.1.markers.detailed_all$top.markers.detailed_all)

top.level.1.markers.detailed_all.group.5 <- data.frame(cbind(group.5.names, 'Group 5', TRUE))
colnames(top.level.1.markers.detailed_all.group.5) <- colnames(top.level.1.markers.detailed_all$top.markers.detailed_all)

top.level.1.markers.detailed_all.groups <-rbind(top.level.1.markers.detailed_all.group.1,
                                                 top.level.1.markers.detailed_all.group.2, 
                                                 top.level.1.markers.detailed_all.group.3,
                                                 top.level.1.markers.detailed_all.group.4,
                                                 top.level.1.markers.detailed_all.group.5)

colnames(top.level.1.markers.detailed_all.groups) <- colnames(top.level.1.markers.detailed_all$top.markers.detailed_all)
rownames(top.level.1.markers.detailed_all.groups) <- top.level.1.markers.detailed_all.groups$gene_name
...
plot.level.top.semireference <- plot_heatmap_markers_multiple_level(
                     ...
                     pseudo.bulk.df.filtered.tpm = sc.pseudo.bulk.data.top,
                     res.object.de = RES.thymus.lfc$treatment_Thy_ALKpos_KO_vs_Thy_WT, 
                     top.markers.detailed_all = top.level.1.markers.detailed_all.plus.groups,
                     clustering_variable = 'cluster_normalized_bigstage',
                     list_cell_types = c(1:7), 
                     row_km_value = 1,
                     column.clusters = paste0('cluster_', 1:7),
                     color_left_groups = c("#D55E00", "#CC79A7", "skyblue", "darkgreen", "grey", 'orange', 'blue', 'brown'), 
                     scale_activated = TRUE, 
                     show_legends = TRUE, 
                     show_row_names.param = FALSE
                     ) 
...
```{r matrix.w, fig.width=6, fig.height=10}

data.matrix.w <- deco.alpha.beta.zero.results.partial.top$w
data.matrix.w <- log1p((data.matrix.w))


bulkdata.extra_marker.w.matrix.top_summary <- Heatmap(data.matrix.w, name = "Cell-type signature",
                                 use_raster = FALSE,
                                 cluster_columns = FALSE,
                                 column_dend_reorder = TRUE,
                                 cluster_rows = TRUE,#
...
plot(bulkdata.extra_marker.w.matrix.top_summary)

#save_plot_as_pdf(bulkdata.extra_marker.w.matrix.top_summary, 'bulkdata.extra_marker.w.matrix.top_summary.pdf', 6, 10)

```

```{r matrix.w.top, fig.width=6, fig.height=50}

data.matrix.w <- deco.alpha.beta.zero.results.partial.top$w
data.matrix.w <- log1p((data.matrix.w))


bulkdata.extra_marker.w.matrix.top <- Heatmap(data.matrix.w, name = "Cell-type signature",
                                 use_raster = FALSE,
                                 cluster_columns = FALSE,
                                 column_dend_reorder = TRUE,
                                 cluster_rows = TRUE,#
...
plot(bulkdata.extra_marker.w.matrix.top)

#save_plot_as_pdf(bulkdata.extra_marker.w.matrix.top, 'bulkdata.extra_marker.w.matrix.top.pdf', 6, 50)

```


Now I will analyze the markers of the reference matrix with an Enritchment analysis.
```{r}
...
#save(deco.alpha.beta.zero.results.partial.cds, file = paste0('/Users/crhisto/Documents/GitHub/DecoFlex/DecoFlexR/vignettes/figures_manuscript/', 'deco.alpha.beta.zero.results.partial.cds.Rdata'))
...
plot_proportions_cds_semireference.1.unknown <- ggplot(thymus.proportions.deconvolution.summarized.melt[thymus.proportions.deconvolution.summarized.melt$type_normalized %in% c('Ctrl', 'KO', 'ALK', 'ALKKO', '6_8w_thymus') & thymus.proportions.deconvolution.summarized.melt$cluster_name %in% list_pseudotime,], aes(x=cluster_name, y=value, color=type_normalized)) +
  geom_point(size=4, shape=20, alpha = .30, position=position_jitter(width=.3,height=.00)) +
  labs(title=NULL,x="Cell Type", y = "Proportion") + theme_linedraw() + 
    theme(axis.text.x = element_text(angle = 45, hjust = 1), 
        axis.text=element_text(size=16),
...
plot_proportions_cds_semireference.1.unknown <- plot_proportions_cds_semireference.1.unknown +  scale_color_manual(name = "Sample", breaks = c('6_8w_thymus', 'Ctrl', 'KO', 'ALK', 'ALKKO'), values = c("darkgrey", "#136c38", "blue", "red", "orange")) + theme_light()

plot(plot_proportions_cds_semireference.1.unknown)

#save_plot_as_pdf(plot_proportions_cds_semireference.1.unknown, 'plot_proportions_cds_semireference.1.unknown.pdf', 8, 5)
...
```{r heatmap.level.top.semireference, fig.height=34, fig.width=15}
...
```{r matrix.w.cds, fig.width=6, fig.height=50}

data.matrix.w.cds <- deco.alpha.beta.zero.results.partial.cds$w
data.matrix.w.cds <- log1p((data.matrix.w.cds))


bulkdata.extra_marker.w.matrix.cds <- Heatmap(data.matrix.w.cds, name = "Cell-type signature",
                                 use_raster = FALSE,
                                 cluster_columns = FALSE,
                                 column_dend_reorder = TRUE,
                                 cluster_rows = TRUE,#
...
plot(bulkdata.extra_marker.w.matrix.cds)

#save_plot_as_pdf(bulkdata.extra_marker.w.matrix.cds, 'bulkdata.extra_marker.w.matrix.cds.pdf', 6, 50)
...
unknown_celltypes.dp <- 4

# I have n genes as rows and m+1 columns, being the last one the unknown one. 
mask_h <- matrix(FALSE, nrow = (4 + unknown_celltypes.dp), ncol = 12)
colnames(mask_h) <- standard_name_thymus
rownames(mask_h) <- c(paste0('cluster_', c(2:5)), paste0('unknown_', 1:unknown_celltypes.dp))
...
if(version=='one'){
  ...
  #Version 1.
  mask_h[1:(4+unknown_celltypes.dp),1:3] <- TRUE
}
...
#Now let's put the proportions that I needed, filling with zeros the rest
partial_h_fixed <- matrix(0, nrow = (4 + unknown_celltypes.dp), ncol = 12)
colnames(partial_h_fixed) <- standard_name_thymus
rownames(partial_h_fixed) <- c(paste0('cluster_', c(2:5)), paste0('unknown_', 1:unknown_celltypes.dp))
#let's take the proportion
proportions_wt_single_cell <- as.data.frame(deco.simulation.thymus.tree_guided.recursive.reduced.results.cds$pseudo.bulk.data$truep) 
colnames(proportions_wt_single_cell) <- c('sample_6_8W')
proportions_wt_single_cell <- t(proportions_wt_single_cell)
...
sum(proportions_wt_single_cell['sample_6_8W', paste0('cluster_', c(2:5))] )
...
mask_w <- matrix(TRUE, 
                 nrow = (length(final_markers_partial_model) + length(extra_marker_genes_semireference)), 
                 ncol = (4 + unknown_celltypes.dp))
colnames(mask_w) <- c(paste0('cluster_', c(2:5)), paste0('unknown_', 1:unknown_celltypes.dp))
rownames(mask_w) <- c(final_markers_partial_model, extra_marker_genes_semireference)
mask_w[1:length(final_markers_partial_model), (4+1):(4+unknown_celltypes.dp)] <- FALSE
mask_w[(length(final_markers_partial_model)+1):nrow(mask_w), 1:(4+unknown_celltypes.dp)] <- FALSE
...
partial_w_fixed <- matrix(0, 
                          nrow = (length(final_markers_partial_model) + length(extra_marker_genes_semireference)), 
                          ncol = (4 + unknown_celltypes.dp))
colnames(partial_w_fixed) <- c(paste0('cluster_', c(2:5)), paste0('unknown_', 1:unknown_celltypes.dp))
rownames(partial_w_fixed) <-  c(final_markers_partial_model, extra_marker_genes_semireference)
...
#save(deco.alpha.beta.zero.results.partial.dp, file = paste0('/Users/crhisto/Documents/GitHub/DecoFlex/DecoFlexR/vignettes/figures_manuscript/', 'deco.alpha.beta.zero.results.partial.dp_four_unknown.Rdata'))
#load(paste0('/Users/crhisto/Documents/GitHub/DecoFlex/DecoFlexR/vignettes/figures_manuscript/', 'deco.alpha.beta.zero.results.partial.dp_one_unknown.Rdata'))
```
...
order_normalized <- c(paste0('cluster_', c(2:5)), paste0('unknown_', 1:unknown_celltypes.dp))
order_samples <- c('sample_6_8W')
list_pseudotime <- c("2preDP", "3DPbla", "4DPre", "5DPsel", paste0('unknown_', 1:unknown_celltypes.dp))
...
#Initialization of extra clusters unknown
for(extra in 1:unknown_celltypes.dp){
  names_temp <- c(colnames(real_proportion), paste0('unknown_', extra))
  real_proportion <- cbind(real_proportion, 0)
  colnames(real_proportion) <- names_temp
}
...
plot_proportions_dp_semireference.1.unknown <- ggplot(thymus.proportions.deconvolution.summarized.melt[thymus.proportions.deconvolution.summarized.melt$type_normalized %in% c('Ctrl', 'KO', 'ALK', 'ALKKO', '6_8w_thymus') & thymus.proportions.deconvolution.summarized.melt$cluster_name %in% list_pseudotime,], aes(x=cluster_name, y=value, color=type_normalized)) +
  geom_point(size=4, shape=20, alpha = .30, position=position_jitter(width=.3,height=.00)) +
  labs(title=NULL,x="Cell Type", y = "Proportion") + theme_linedraw() + 
    theme(axis.text.x = element_text(angle = 45, hjust = 1), 
        axis.text=element_text(size=16),
...
plot_proportions_dp_semireference.1.unknown <- plot_proportions_dp_semireference.1.unknown +  scale_color_manual(name = "Sample", breaks = c('6_8w_thymus', 'Ctrl', 'KO', 'ALK', 'ALKKO'), values = c("darkgrey", "#136c38", "blue", "red", "orange")) + theme_light()

plot_proportions_dp_semireference.1.unknown

#save_plot_as_pdf(plot_proportions_dp_semireference.1.unknown, 'plot_proportions_dp_semireference.1.unknown.pdf', 8, 5)
...
```{r matrix.w.dp, fig.width=6, fig.height=65}

data.matrix.w.dp <- deco.alpha.beta.zero.results.partial.dp$w
data.matrix.w.dp <- log1p((data.matrix.w.dp))


bulkdata.extra_marker.w.matrix.dp <- Heatmap(data.matrix.w.dp, name = "Cell-type signature",
                                 use_raster = FALSE,
                                 cluster_columns = FALSE,
                                 column_dend_reorder = TRUE,
                                 cluster_rows = TRUE,#
...
plot(bulkdata.extra_marker.w.matrix.dp)

#save_plot_as_pdf(bulkdata.extra_marker.w.matrix.dp, 'bulkdata.extra_marker.w.matrix.dp.pdf', 6, 65)
